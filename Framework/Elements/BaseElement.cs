...
namespace demo.framework.Elements
{
    public abstract class BaseElement : BaseEntity
    {
        ...
        public void ClickByMouseOver()
        {
            WaitForElementPresent();
            new Actions(Browser.GetDriver()).MoveToElement(GetElement()).Click(GetElement()).Perform();
        }
        ...
        public static void WaitForElementPresent(By locator, String name)
        {
            ...
            {
                Log.Fatal(string.Format("Element with locator: '{0}' does not exists!", locator.ToString()));
            }
        }
